<script module lang="ts">
	export const THEME_PARTS = [ 'fonts', 'theme', 'variables', 'typography',  'layout', 'form' ] as const;
...
<script lang="ts">
	...
	const defaultThemeParts: ThemePartsType[] = [ 'fonts', 'theme', 'variables', 'typography', 'layout', 'form' ];

	let { themeData }: ThemePropsType = $props();
	let themeParts: ThemePartsType[] = $state(Array.isArray( themeData?.parts ) && themeData.parts.length > 0 ? themeData.parts : defaultThemeParts);
	...
	export const setBaseValues = ( data: ThemeDataType & { mode?: ThemeModeType } | undefined ) => {
		...
		currentMode = tmpMode ? ( tmpMode as ThemeModeType ) : toLocalStorage( 'mode', data.mode || 'light' ) as ThemeModeType;
		...
		currentDark = tmpDark ? ( tmpDark as ThemeDataType['dark'] ) : toLocalStorage( 'dark', data.dark || {} ) as ThemeDataType['dark'];

		const tmpLight = getFromLocalStorage( 'light' );
		currentLight = tmpLight ? ( tmpLight as ThemeDataType['light'] ) : toLocalStorage( 'light', data.light || {} ) as ThemeDataType['light'];
	};
